...
Agent-panel forks use a different launch mode. Supaterm starts the account login shell and enters the agent's native fork command visibly. Claude, Codex, and Pi forks keep supported launch options from the source process. The pane returns to that same shell when the forked agent exits.
